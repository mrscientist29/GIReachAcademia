...
import { GraduationCap, FileText, BookOpen, Users, Award, ArrowRight, Target, Eye, Heart } from "lucide-react";
...
export default function AboutDynamic() {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        console.log('AboutDynamic: Loading content...');
        
        // Always try async first for fresh data
        const content = await contentStore.getPageContent('about');
        console.log('AboutDynamic: Loaded content:', content);
        
        if (mounted) {
          if (content && content.sections && content.sections.length > 0) {
            setPageContent(content);
            console.log(`AboutDynamic: Successfully loaded ${content.sections.length} sections`);
          } else {
            console.warn('AboutDynamic: Content missing sections, trying sync method');
            const fallbackContent = contentStore.getPageContentSync('about');
            if (fallbackContent && fallbackContent.sections) {
              setPageContent(fallbackContent);
              console.log(`AboutDynamic: Used fallback content with ${fallbackContent.sections.length} sections`);
            } else {
              console.error('AboutDynamic: No valid content found');
            }
          }
        }
      } catch (error) {
        console.error('AboutDynamic: Error loading content:', error);
        if (mounted) {
          const fallbackContent = contentStore.getPageContentSync('about');
          console.log('AboutDynamic: Fallback content:', fallbackContent);
          if (fallbackContent) {
            setPageContent(fallbackContent);
          }
        }
    ...
    const handleContentUpdate = (e: CustomEvent) => {
      if (e.detail.pageId === 'about' && mounted) {
        console.log('AboutDynamic: Content updated via event', e.detail.content);
    ...
    switch (section.type) {
      case 'hero':
        return (
          ...
          >
            <div className={`absolute inset-0 ${section.styles?.backgroundColor?.includes('from-') ? '' : (section.styles?.backgroundColor || 'bg-gradient-to-r from-green-50 to-emerald-100')}`}></div>
