...
import { GraduationCap, FileText, BookOpen, Users, Award, ArrowRight, Calendar, Clock, MapPin } from "lucide-react";
...
export default function ProgramsDynamic() {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        console.log('ProgramsDynamic: Loading content...');
        
        const content = await contentStore.getPageContent('programs');
        console.log('ProgramsDynamic: Loaded content:', content);
        
        if (content && content.sections) {
          setPageContent(content);
        } else {
          console.warn('ProgramsDynamic: Content missing sections, trying sync method');
          const fallbackContent = contentStore.getPageContentSync('programs');
          setPageContent(fallbackContent);
        }
      } catch (error) {
        console.error('ProgramsDynamic: Error loading content:', error);
        const fallbackContent = contentStore.getPageContentSync('programs');
        console.log('ProgramsDynamic: Fallback content:', fallbackContent);
        setPageContent(fallbackContent);
      }
    };
    ...
    const handleContentUpdate = (e: CustomEvent) => {
      if (e.detail.pageId === 'programs') {
        console.log('ProgramsDynamic: Content updated via event');
    ...
    switch (section.type) {
      case 'hero':
        return (
          ...
          >
            <div className={`absolute inset-0 ${section.styles?.backgroundColor?.includes('from-') ? '' : (section.styles?.backgroundColor || 'bg-gradient-to-r from-purple-50 to-pink-100')}`}></div>
              ...
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <Link href="/join">
                      <Button 
                        className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-4 text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
