...
export async function setupAuth(app: Express) {
  ...
  if (process.env.NODE_ENV?.trim() === 'development' || process.env.REPLIT_DOMAINS === 'localhost:5001') {
    console.log("Skipping Replit auth setup for local development");

    // Simple auth routes for development
    app.get("/api/login", (req, res) => {
      res.json({ message: "Login not available in development mode" });
    });
    ...
    // Set up simple serialization for development
